...
serve(async (req) => {
  try {
    ...
    console.log(`🔄 Processing background task: ${taskId}`)
    ...
    if (taskError || !taskData) {
      console.error('Task not found or already processed:', taskError)
      return new Response(
        JSON.stringify({ error: 'Task not found or already processed' }),
    ...
    return new Response(
      JSON.stringify({ success: true, message: 'Article generated successfully' }),
    ...
    // Update task as failed if we have a task_id
    const url = new URL(req.url)
    const taskId = url.searchParams.get('task_id')
    ...
    return new Response(
      JSON.stringify({ error: 'Background processing failed', message: error.message }),
