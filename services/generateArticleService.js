// JavaScript version of article generation service for Netlify functions
...
function selectBestTitle(titleVariations, primaryKeyword) {
...
function validateArticleResponse(response) {
  try {
...
function isResponseComplete(responseText) {
  try {
...
async function getKeywordVolume(keyword, location) {
...
async function generateSeoGeoArticle(topic, location, tone, brief, enableInternalLinks, websiteUrl, internalLinksContext, seoKeywords = []) {
...
// Helper function for FAQ validation (simplified)
function validateFaqFormat(articleContent) {
  try {
    ...
    for (const line of lines) {
      ...
      if ((trimmed === '## FAQs' || trimmed === '## FAQ') && !inFaqSection) {
        inFaqSection = true;
        continue;
      }
